...
`Identity -> Memory -> Context -> Behavior -> Experience -> Observer -> Reflection/Consolidation -> reviewed promotion -> Memory + Identity -> future behavior`
...
2. An adapter retrieves completed structural evidence and validated `celestan-semantic-evidence-envelope-v1` claims. Observer seals them in an immutable `celestan-observer-evidence-join-v1` before creating a provider-neutral semantic task. Structural context is factual activity context with no independent semantic authority. Celestan's normal model-selection machinery performs inference; Observer validates the binding hash and cited claim IDs and records failures without claiming observation.
3. Observer writes exactly one immutable `celestan-execution-digest-v1` record per execution. Records with an eligible join are `semantic-analysis-pending`; structural-only and legacy reasoning records are `semantic-evidence-insufficient` and never count as semantically observed. The filename and `ledger.ndjson` provide idempotency; `cursor.json` is a restart hint, while records remain authoritative if a crash occurs before checkpoint update.
...
The runtime must maintain a manifest of expected executions. `coverageReport()` identifies missing evidence records, eligible records still pending semantic observation, semantically ineligible records, and adapter/Observer failures. `joinedRecords(store)` and `store.joined()` expose the one authoritative view containing every digest plus a validated, join-bound semantic projection when a sidecar exists. Digest-owned schema/version, execution identity/project/status/timestamps, provenance, observation timestamp, and measurements always win. Consolidation and Chronicle filter the joined view to observed records; structural-only records never become semantic conclusions. Re-running a digest is safe and returns `duplicate`; malformed, unsupported, uncited, or low-confidence semantic output is rejected and can be recorded as a retryable lifecycle failure.
