...
if (!command || !['digest', 'consolidate', 'chronicle'].includes(command)) {
  console.error('Usage: observer.mjs digest --input evidence.json [--semantic semantic.json] [--store path] | consolidate | chronicle --start YYYY-MM-DD --end YYYY-MM-DD');
  process.exit(2);
}
if (command === 'digest') {
  const input = JSON.parse(await readFile(option('--input'), 'utf8'));
  const semanticPath = option('--semantic');
  const semantic = semanticPath ? JSON.parse(await readFile(semanticPath, 'utf8')) : {};
