...
| `RAD-003` | External agent-skill ecosystem survey | Evaluated | [skills.sh leaderboard](https://skills.sh/) (checked 2026-08-27), [OpenCode skills documentation](https://opencode.ai/docs/skills/), GitHub repository search, Reddit search attempted | External distribution signals may reveal broadly reusable verification, debugging, browser, and development skills before local friction is visible. | Revisit a named candidate only with a concrete Celestan task, source review, isolated installation, and representative verification. | Not installed |
...
- **Question:** Does the external agent-skill ecosystem reveal a broadly reusable capability worth evaluating for Celestan now?
- **Cost:** The leaderboard is publicly accessible and discovery required no installation or paid commitment. Skill installation would add repository trust, prompt/context, dependency, and maintenance costs.
- **Sources and signals:** On 2026-08-27, skills.sh listed `find-skills`, `tdd`, `frontend-design`, `improve-codebase-architecture`, `agent-browser`, `verification-before-completion`, `systematic-debugging`, and `code-review` among high-install skills. GitHub search surfaced active repositories including `vercel-labs/agent-browser`, `obra/superpowers`, `mattpocock/skills`, and `joshuadavidthomas/opencode-agent-skills`. Reddit search pages were reachable, but Reddit search/API responses did not provide usable result evidence in this environment.
- **Candidate assessment:** `agent-browser` reports persistent sessions, accessibility-tree snapshots, screenshots, extraction, and JavaScript execution, but overlaps the unresolved Playwright browser frontier and has a Snyk warning on its listing; no install is justified. `tdd` emphasizes public-interface behavior tests and vertical slices, while `systematic-debugging` mandates root-cause investigation and `verification-before-completion` requires fresh proof before completion claims. These align with existing Foundry and project practices, but popularity does not prove that importing external prompt text improves Celestan.
- **Security and constraints:** Leaderboard counts are distribution signals, not quality benchmarks. Third-party skills may contain instruction conflicts, unsafe commands, stale assumptions, or supply-chain risk. OpenCode discovers skills from project and global directories and supports permission patterns; any future installation must be pinned, reviewed, isolated, and permissioned.
- **Decision:** Remain `Evaluated`, not `Available`. The survey establishes a repeatable external-intelligence input and a shortlist, but no candidate currently demonstrates enough incremental action space or Celestan-specific evidence for promotion. Prefer adopting principles selectively through existing instructions unless a representative task shows measurable benefit from a concrete skill.
