...
## EXP-010 - additive model/runtime telemetry contract

- **Question:** Can Observer preserve useful provider/model/runtime facts and derive deterministic covered aggregates without converting activity metrics into model quality or routing claims?
- **Hypothesis:** Strict safe normalization, invocation-first aggregation, and a separate evidence-citing semantic routing assessment can support future runtime learning without double counting, hidden missing values, currency mixing, or a universal score.
- **Smallest test:** Exercise absent, zero, partial, session-only, mixed-currency, failure, transition, cross-reference, unsafe-field, semantic-reference, immutable-sidecar, and task-conditioned consolidation cases against one comprehensive fixture.
- **Success signal:** Every mechanical dimension retains basis/coverage; infrastructure facts stay factual; semantic attribution is explicit; routing candidates remain review-only and task-conditioned.
- **Failure signal:** Missing values become zero, sessions and invocations are summed together, raw records admit unsafe content, metrics emit policy signals, or routing changes can be auto-accepted.
- **Current result:** 23 Observer tests pass. OpenCode exposes useful message/task/tool facts but lacks authoritative retry/fallback intent, context limit, and session finish; observed local zero costs are unavailable absent an explicit measured assertion. The contract is `Available` for future CT-Runtime integration, with no automatic optimization or enforcement.
