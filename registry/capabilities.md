...
| --- | --- | --- | --- | --- |
| `repo-state-inspector` | Preferred | [`capabilities/repo-state-inspector`](../capabilities/repo-state-inspector/), [`C:\Celestan\capabilities\repo-state-inspector`](file:///C:/Celestan/capabilities/repo-state-inspector/) | Produce a compact, read-only report of repository structure, state-file signals, nested project markers, package metadata, verification entry points, CI workflows, and Git status. | 4 local tests pass; seven real repository runs found project state/CI, nested Node/.NET/Terraform surfaces, inaccessible Git metadata, and a repository with no durable state file; directly discoverable entry point installed. |
